...
class TaskSchedulingControls @JvmOverloads constructor(
    ...
    var frequency = TaskEnum.FREQUENCY_DAILY
    set(value) {
        field = value
        repeatsEverySpinner.setSelection(when (value) {
            TaskEnum.FREQUENCY_WEEKLY -> 1
            TaskEnum.FREQUENCY_MONTHLY -> 2
            TaskEnum.FREQUENCY_YEARLY -> 3
            else -> 0
        })
    ...
    init {
        ...
        frequency = TaskEnum.FREQUENCY_WEEKLY
        startDate = Date()
        everyX = 1
        weeklyRepeat = Days()
        ...
        repeatsEverySpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            ...
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                frequency = when (position) {
                    1 -> TaskEnum.FREQUENCY_WEEKLY
                    2 -> TaskEnum.FREQUENCY_MONTHLY
                    3 -> TaskEnum.FREQUENCY_YEARLY
                    else -> TaskEnum.FREQUENCY_DAILY
                }
            }
        }
        ...
        startDateWrapper.setOnClickListener {
            ...
            datePickerDialog.setButton(DialogInterface.BUTTON_NEUTRAL, resources.getString(R.string.today)) { _, _ ->
                if (taskType == TaskEnum.TYPE_TODO) {
                    dueDate = Date()
                } else {
                    startDate = Date()
                }
    ...
    private fun configureViewsForType() {
        startDateTitleView.text = context.getString(if (taskType == TaskEnum.TYPE_DAILY) R.string.start_date else R.string.due_date)
        repeatsEveryWrapper.visibility = if (taskType == TaskEnum.TYPE_DAILY) View.VISIBLE else View.GONE
        summaryTextView.visibility =  if (taskType == TaskEnum.TYPE_DAILY) View.VISIBLE else View.GONE
        weeklyRepeatWrapper.visibility = if (taskType == TaskEnum.TYPE_DAILY) View.VISIBLE else View.GONE
    }
    ...
    private fun configureViewsForFrequency() {
        repeatsEveryTitleView.text = context.getText(when (frequency) {
            TaskEnum.FREQUENCY_WEEKLY -> R.string.weeks
            TaskEnum.FREQUENCY_MONTHLY -> R.string.months
            TaskEnum.FREQUENCY_YEARLY -> R.string.years
            else -> R.string.days
        })
        weeklyRepeatWrapper.visibility = if (frequency == TaskEnum.FREQUENCY_WEEKLY && taskType == TaskEnum.TYPE_DAILY) View.VISIBLE else View.GONE
        monthlyRepeatWrapper.visibility = if (frequency == TaskEnum.FREQUENCY_MONTHLY && taskType == TaskEnum.TYPE_DAILY) View.VISIBLE else View.GONE
        if (frequency == TaskEnum.FREQUENCY_WEEKLY) {
            createWeeklyRepeatViews()
        } else if (frequency == TaskEnum.FREQUENCY_MONTHLY) {
